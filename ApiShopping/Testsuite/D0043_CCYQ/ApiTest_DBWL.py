...
@author: 18056678（郭海龙）
@project: ApiShopping
@file: ApiTest_HBYC.py
@time: 2020-03-31 17:26:37
@desc:
'''
from API.D0043_CCYQ.Base import API
api = API(envi='pre',user='DBWL',isLog='N')
skus = '121347746'
# 接收采购平台订单消息 = api.handMsg(business='ORDER',event='ADD',pcode='DB202006218001')
...
# 获取价格及税率 = api.getProdPriceAndTaxcode(skus=skus,provinceCode='110000000000',citycode='110100000000',countyCode='110101000000')
# 校验商品售卖区域 = api.checkSalearea(skus=skus,provinceCode='110000000000',citycode='110100000000',countyCode='110101000000',townCode='')
# 获取商品上下架状态 = api.getShelvesStatus(skus=skus)
# 校验商品是否有库存 = api.getStockStatus(skus=skus,nums='30',provinceCode='320000000000',citycode='320100000000',countyCode='320102000000',townCode='320102002000')